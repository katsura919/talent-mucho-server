...
import { updateAdminProfileSchema, updateAdminEmailSchema, updateAdminPasswordSchema, } from "../../../types/admin.types.js";
// PATCH /admin/settings/profile – update firstName and/or lastName
export async function updateAdminProfile(request, reply) {
    ...
    const result = await admins.findOneAndUpdate({ _id: new ObjectId(id) }, { $set: updateFields }, { returnDocument: "after" });
    if (!result) {
...
export async function updateAdminEmail(request, reply) {
    const admins = request.server.mongo.db?.collection("admins");
    if (!admins) {
        return reply.status(500).send({ error: "Database not available" });
    }
    ...
    const result = await admins.findOneAndUpdate({ _id: new ObjectId(id) }, { $set: { email, updatedAt: new Date().toISOString() } }, { returnDocument: "after" });
    if (!result) {
        return reply.status(404).send({ error: "Admin not found" });
    }
    const { password, ...adminWithoutPassword } = result;
...
export async function updateAdminPassword(request, reply) {
    const admins = request.server.mongo.db?.collection("admins");
    if (!admins) {
        return reply.status(500).send({ error: "Database not available" });
    }
    ...
    await admins.updateOne({ _id: new ObjectId(id) }, { $set: { password: hashedPassword, updatedAt: new Date().toISOString() } });
