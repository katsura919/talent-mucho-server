...
import { createCompensationProfileSchema, updateCompensationProfileSchema, updateStaffStatutorySettingsSchema, } from "../../types/compensation-profile.types.js";
...
export async function updateCompensationProfile(request, reply) {
    const profiles = request.server.mongo.db?.collection("compensation_profiles");
    if (!profiles) {
        return reply.status(500).send({ error: "Database not available" });
    }
    ...
    const result = await profiles.findOneAndUpdate({ _id: new ObjectId(id) }, {
        $set: {
            ...updates,
...
export async function getCompensationProfiles(request, reply) {
    const profiles = request.server.mongo.db?.collection("compensation_profiles");
    const businesses = request.server.mongo.db?.collection("businesses");
...
export async function updateStaffStatutorySettings(request, reply) {
    const profiles = request.server.mongo.db?.collection("compensation_profiles");
