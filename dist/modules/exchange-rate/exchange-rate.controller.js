...
export async function getExchangeRates(request, reply) {
...
export async function getExchangeRate(request, reply) {
    const exchangeRates = request.server.mongo.db?.collection("exchange_rates");
    if (!exchangeRates) {
        return reply.status(500).send({ error: "Database not available" });
    }
...
export async function upsertExchangeRate(request, reply) {
    const exchangeRates = request.server.mongo.db?.collection("exchange_rates");
    if (!exchangeRates) {
        return reply.status(500).send({ error: "Database not available" });
    }
...
export async function deleteExchangeRate(request, reply) {
    const exchangeRates = request.server.mongo.db?.collection("exchange_rates");
    if (!exchangeRates) {
        return reply.status(500).send({ error: "Database not available" });
    }
