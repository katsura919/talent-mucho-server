...
export async function generateInvoice(request, reply) {
    ...
    const financials = calculateInvoiceFinancials(eodRecords, compensation, [], [], periodEnd, compensation.currency);
...
export async function recalculateInvoice(request, reply) {
    ...
    const financials = calculateInvoiceFinancials(eodRecords, compensation, invoice.additions || [], invoice.deductions || [], invoice.periodEnd, compensation.currency);
    const now = new Date().toISOString();
    ...
    const result = await invoices.findOneAndUpdate({ _id: new ObjectId(id) }, { $set }, { returnDocument: "after" });
    return {
        ...result,
...
export async function getInvoicesByBusiness(request, reply) {
    const db = request.server.mongo.db;
    const invoices = db?.collection("invoices");
    const businesses = db?.collection("businesses");
    const staffCollection = db?.collection("staff");
...
export async function getInvoicesByStaff(request, reply) {
    const db = request.server.mongo.db;
    const invoices = db?.collection("invoices");
    const staff = db?.collection("staff");
    const businesses = db?.collection("businesses");
...
// ==================== INVOICE ACTIONS ====================
// Approve invoice
export async function approveInvoice(request, reply) {
...
export async function markInvoicePaid(request, reply) {
    const invoices = request.server.mongo.db?.collection("invoices");
    const businesses = request.server.mongo.db?.collection("businesses");
    if (!invoices || !businesses) {
        return reply.status(500).send({ error: "Database not available" });
...
export async function addInvoiceAdjustment(request, reply) {
    const invoices = request.server.mongo.db?.collection("invoices");
    const businesses = request.server.mongo.db?.collection("businesses");
    if (!invoices || !businesses) {
        return reply.status(500).send({ error: "Database not available" });
    ...
    if (invoice.status === "approved") {
        return reply.status(400).send({
            ...
            message: "This invoice has been approved. Revoke approval first to add adjustments.",
        });
    }
    // Check business access (unless super-admin)
    if (request.user.role !== "super-admin") {
    ...
    const deductionsTotal = type === "deduction"
        ? [...invoice.deductions, adjustment].reduce((sum, d) => sum + d.amount, 0)
        : invoice.deductions.reduce((sum, d) => sum + d.amount, 0);
    const additionsTotal = type === "addition"
        ? [...invoice.additions, adjustment].reduce((sum, a) => sum + a.amount, 0)
        : invoice.additions.reduce((sum, a) => sum + a.amount, 0);
...
// Remove adjustment
export async function removeInvoiceAdjustment(request, reply) {
    const invoices = request.server.mongo.db?.collection("invoices");
    const businesses = request.server.mongo.db?.collection("businesses");
    if (!invoices || !businesses) {
        return reply.status(500).send({ error: "Database not available" });
    ...
    // Recalculate net pay
    const deductionsTotal = type === "deduction"
        ? adjustments.reduce((sum, d) => sum + d.amount, 0)
        : invoice.deductions.reduce((sum, d) => sum + d.amount, 0);
    const additionsTotal = type === "addition"
...
export async function deleteInvoice(request, reply) {
    ...
    await invoices.findOneAndUpdate({ _id: new ObjectId(id) }, { $set: { isActive: false, updatedAt: new Date().toISOString() } });
