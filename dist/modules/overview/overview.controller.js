...
export async function getBusinessOverviewStats(request, reply) {
    ...
    const [totalStaff, openPositions, recentEods7d, pendingEodApprovals] = await Promise.all([
...
export async function getRecentBusinessInvoices(request, reply) {
    const db = request.server.mongo.db;
...
export async function getRecentBusinessEods(request, reply) {
    const db = request.server.mongo.db;
    const eodReports = db?.collection("eod_reports");
    if (!db || !eodReports) {
        return reply.status(500).send({ error: "Database not available" });
