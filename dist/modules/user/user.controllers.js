...
export async function deleteUser(request, reply) {
    ...
    const result = await users.findOneAndUpdate({ _id: new ObjectId(id) }, { $set: { isActive: false, updatedAt: new Date().toISOString() } }, { returnDocument: 'after' });
    if (!result) {
        return reply.status(404).send({ error: 'User not found' });
    }
...
export async function hardDeleteUser(request, reply) {
    const users = request.server.mongo.db?.collection('users');
    if (!users) {
        return reply.status(500).send({ error: 'Database not available' });
    }
