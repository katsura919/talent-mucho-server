...
import { calculateInvoiceFinancials, resolveHourlyCompensationProfile, } from "../modules/invoice/invoice.calculator.service.js";
...
async function generateInvoicesForDateRange(fastify, periodStart, periodEnd) {
    ...
    for (const staffMember of staffMembers) {
        try {
            ...
            result.generated++;
            result.details.push(`Generated invoice for ${staffName}: ${periodStart} to ${periodEnd} (${financials.totalHoursWorked}h, $${financials.calculatedPay})`);
        }
        catch (err) {
            result.errors++;
            const staffName = `${staffMember.firstName} ${staffMember.lastName}`;
...
const cronPlugin = async (fastify) => {
    cron.schedule("0 0 1 * *", async () => {
        fastify.log.info("[CRON] Generating invoices for previous month second half...");
        try {
    ...
    cron.schedule("0 0 16 * *", async () => {
        fastify.log.info("[CRON] Generating invoices for current month first half...");
        try {
