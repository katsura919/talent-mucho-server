...
import {
  updateAdminProfileJsonSchema,
  updateAdminEmailJsonSchema,
  updateAdminPasswordJsonSchema,
  adminJsonSchema,
...
const adminSettingsRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.patch(
    ...
    updateAdminProfile as AnyHandler,
  );
  ...
  fastify.patch(
    "/admin/settings/email",
    {
      ...
      schema: {
        description:
          "Update authenticated admin's email (requires current password)",
        tags: ["Admin Settings"],
        security: [{ bearerAuth: [] }],
    ...
    updateAdminEmail as AnyHandler,
  );
  ...
  fastify.patch(
    ...
    updateAdminPassword as AnyHandler,
  );
};
