...
export async function getMyExpectedEarnings(
  ...
  const totalHoursWorked = approvedEods.reduce(
...
export async function editOwnEod(
  ...
  if (existingEod.status !== "needs_revision") {
    return reply.status(403).send({
      error: "Cannot edit",
      message:
        "You can only edit an EOD report that has been returned for revision",
    });
  }
  ...
  const result = await eodReports.findOneAndUpdate(
    { _id: new ObjectId(id) },
    { $set: updateData },
...
export async function deleteEod(
  ...
  await eodReports.findOneAndUpdate(
    { _id: new ObjectId(id) },
