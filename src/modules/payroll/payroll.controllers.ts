...
export async function generatePayroll(
    ...
    const uniqueWorkDates = new Set(
        attendanceRecords.map(record => record.workDate)
    );
    ...
    // Calculate pay
    const calculatedPay = calculatePay(
...
export async function generateBusinessPayroll(
...
export async function getPayrollByStaff(
    request: FastifyRequest<{ Params: StaffIdParams; Querystring: PayrollQuery }>,
    reply: FastifyReply,
) {
    const payroll = request.server.mongo.db?.collection("payroll");
    const staff = request.server.mongo.db?.collection("staff");
...
export async function getPayrollByBusiness(
    request: FastifyRequest<{ Params: BusinessIdParams; Querystring: PayrollQuery }>,
    reply: FastifyReply,
) {
    const payroll = request.server.mongo.db?.collection("payroll");
    const businesses = request.server.mongo.db?.collection("businesses");
    ...
    const staffMap = new Map(
        staffMembers.map(s => [s._id.toString(), s])
    );
...
export async function addPayrollAdjustment(
    ...
    const deductionsTotal = type === 'deduction'
        ...
        : payrollRecord.deductions.reduce((sum: number, d: any) => sum + d.amount, 0);

    const additionsTotal = type === 'addition'
        ? [...payrollRecord.additions, adjustment].reduce((sum, a) => sum + a.amount, 0)
        : payrollRecord.additions.reduce((sum: number, a: any) => sum + a.amount, 0);
