...
import {
  staffJsonSchema,
  updateStaffProfileJsonSchema,
  addStaffDocumentJsonSchema,
...
const staffRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.patch<{
    ...
    "/staff/me/profile",
    {
      ...
      schema: {
        description:
          "Update current staff member's profile (firstName, lastName, phone)",
        tags: ["Staff"],
        security: [{ bearerAuth: [] }],
  ...
  fastify.post<{ Body: { name: string; url: string; type: string } }>(
    "/staff/me/documents",
    {
      preHandler: [fastify.authenticateStaff],
      schema: {
        description:
          "Add a document to the current staff member's profile using a URL (e.g. Google Drive)",
        tags: ["Staff"],
        security: [{ bearerAuth: [] }],
